...
jQuery.extend({
	...
	sendAjaxReq: function(url, data, callback, async, postDataType, utf8Encode)
	{
		if (true == $.local)
		{
			return callback && callback({err_code: ENONE, error_code: ENONE});
		}
	...
	getLocalData: function(data, callback, async, method)
	{
		...
		for (var uciObj in data)
		{
			if ("method" != uciObj)
			{
				var secObj = data[uciObj]["name"] || data[uciObj]["table"];
				ret[uciObj] = {};
	...
	query: function(data, callback, async)
	{
		if ($.local)
		{
			return $.getLocalData(data, callback, async, "get");
		}
		else
		{
...
jQuery.extend({
	...
	/* 不需要认证的通信接口 */
	unAuthSendDsData: function(cmd, callBack, async)
	{
		cmd.method = "do";
	...
	/* 登录使用/修改密码 */
	auth: function(pwd, callBack, asyn)
	{
		var reqData, pwdStr;

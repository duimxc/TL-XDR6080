function Checks()
{
	...
	{
		/* 返回值为true表明是纯数字，false表明不是纯数字或为空 */
		return (/^\d+$/.test(value));
	};
	...
	this.checkInputName = function(value, maxLen, minLen)
	{
		...
		{
			return ESTRINGLEN;	//EOUTOFRANGE已废弃
		}
	...
	/* 检查PIN码: 由8位十进制数字组成 */
	this.checkPin = function(value)
	{
		var result = /^[0-9]{8,8}$/g.test(value);
		return (0 == value.length) ? EINVWPSPINEMPTY : (true == result ? ENONE : EINVWPSPINFORMAT);
	}
}
